...
openssl x509 -req -in $SSLDIR/node.csr -CA $SSLDIR/ca.crt -CAkey $SSLDIR/ca.key -CAcreateserial \
    -out $SSLDIR/node.crt -days $DAYS -extensions v3_req -extfile openssl.conf
...
openssl x509 -req -in $SSLDIR/peer.csr -CA $SSLDIR/ca.crt -CAkey $SSLDIR/ca.key -CAcreateserial \
    -out $SSLDIR/peer.crt -days $DAYS -extensions v3_req -extfile openssl.conf
