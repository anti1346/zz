...
for i in {1..3}; do
    ...
    sed -i "s/8005/$(($http_port - 1))/g" "${instance_dir}/conf/server.xml"
    sed -i "s/8009/$(($http_port + 1))/g" "${instance_dir}/conf/server.xml"
    sed -i "s/8443/$(($http_port + 443))/g" "${instance_dir}/conf/server.xml"

    # Add a new web app (optional)
    if ! [ -f "${instance_dir}/webapps/ROOT/test.jsp" ]; then
        cat <<'EOT' > "${instance_dir}/webapps/ROOT/test.jsp"
